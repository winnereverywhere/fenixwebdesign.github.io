...
function cloneDeep(elm,index){var clone=elm.cloneNode(true)
...
return position}
function cancel(){translate(getPosition(),true)
Transition.cancel()}
...
function init(){slideCount=getLength(true)
...
if(indicator==='+'||indicator==='-'){index=computeDestIndex(currIndex+ +(''+indicator+(+number||1)),currIndex)}else if(indicator==='>'){index=number?toIndex(+number):getNext(true)}else if(indicator==='<'){index=getPrev(true)}}else{index=isLoop?control:clamp(control,0,endIndex)}
return index}
...
created=true
append(wrapper,[prev,next])!placeholder&&before(wrapper,track)}
function createArrow(prev2){var arrow='<button class="'+
classes.arrow+' '+
(prev2?classes.prev:classes.next)+'" type="button"><svg xmlns="'+
XML_NAME_SPACE+'" viewBox="0 0 '+
SIZE+' '+
SIZE+'" width="'+
SIZE+'" height="'+
SIZE+'" focusable="false"><path d="'+
...
function toggle(cover2,img,Slide){Slide.style('background',cover2?'center/cover no-repeat url("'+img.src+'")':'',true)
...
disable(!drag)
isFree=drag==='free'}
function onPointerDown(e){clickPrevented=false
...
return 0}
function computeDestination(velocity){return(getPosition()+
sign(velocity)*min(abs(velocity)*(options.flickPower||600),isFree?Infinity:Components2.Layout.listSize()*(options.flickMaxPages||1)))}
...
function update(){var prev=getAt(getIndex(true))
...
endCallback()}})}
function start(index,done){var destination=Move.toPosition(index,true)
...
try{merge(options,JSON.parse(getAttribute(root,DATA_ATTRIBUTE)))}catch(e){assert(false,'Invalid JSON')}
...
return this}
_proto.destroy=function destroy(completely){if(completely===void 0){completely=true}
